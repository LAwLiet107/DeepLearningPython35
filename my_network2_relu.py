...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 10 12:47:52 2019
...
class Network(object):
    ...
    def __init__(self, sizes, cost = CrossEntropyCost):
        ...
        self.sizes = sizes
        self.relu_weight_initializer()
        self.cost = cost
        self.traincostvalue = []
        self.evaluationaccuracyvalue = []
        
    def relu_weight_initializer(self):
        """初始化weights和biases"""
        self.biases = [np.random.randn(y, 1) for y in self.sizes[1:]]
        self.weights = [np.random.randn(y, x) * np.sqrt(2.0/x) for x, y in zip(self.sizes[:-1], self.sizes[1:])]
        self.velocities = [np.zeros((y, x)) for x, y in zip(self.sizes[:-1], self.sizes[1:])]
    
    def another_relu_weight_initializer(self):
        """另一个初始化方法"""
        self.biases = [np.random.rand(y, 1) - 1/2 for y in self.sizes[1:]]
        self.weights = [np.random.rand(y, x) - 1/2 for x, y in zip(self.sizes[:-1], self.sizes[1:])]
        self.velocities = [np.zeros((y, x)) for x, y in zip(self.sizes[:-1], self.sizes[1:])]
    
    def feedforward(self, a):
        """前向传播，输入及隐藏层使用relu，输出层使用sigmoid"""
        # layers: 1～num_layers-1，relu
        for i in range(self.num_layers - 2):
            # z = w * a + b
            a = relu(np.dot(self.weights[i], a) + self.biases[i])
        # layers: num_layers，sigmoid
        i += 1
